...
3. `MaskedMVACNAdapter` owns `MVACNHead`, omits missing tokens, and takes the
   direct legacy call on four views. Synthetic B and L tests copy weights and
   prove exact (`rtol=atol=0`) full-view equality plus finite 1/2/3-view outputs.
   B uses dropout 0.2/CLS excluded; L uses 0.3/CLS included.
   `compute_mvacn_objective` provides TCP/MSE and correctness/BCE and accepts
   only internally consistent current `CachedTargets`, rejecting stale TCP or
   changed frozen predictions.
...
- `MaskedMVACNAdapter(hidden_by_view, observed, classifier_prediction=[B]) ->`
  `MVACNOutput(confidence_logit [B], confidence [B], observed_mask [B,4])`
...
- `ViLUFailureAdapter(projected_visual_embeddings [B,4,Dv], observed_mask [B,4],
  classifier_prediction [B]) -> ViLUOutput(error/confidence [B], attention [B,4])`
- `compute_vilu_failure_loss(output, CachedTargets) -> scalar`
- `SameInputMLP(raw: RawHeadInputs) -> SameInputOutput([B])`;
...
- `SameInputDensityControl(raw) -> DensityControlOutput(class_logits [B,4],
  frozen_prediction_probability [B])`;
...
matching. DS values are diagnostic features, not validated uncertainty
...
same patients, masks, perturbation draws, optimizer/update/checkpoint budgets,
