...
class InterventionTargets:
    ...
    :attr:`valid_removal_mask`. For valid entries, ``omission_labels`` is the
...
def build_intervention_targets(
    ...
    """Build exact omission targets from the supplied logits and integer labels.
    ...
    with torch.no_grad():
        observed_result = fuse_view_logits(logits_by_view, observed)
        ...
        valid_removal_mask = torch.zeros(
            len(CANONICAL_VIEWS), dtype=torch.bool, device=labels.device
        )
        ...
        if len(observed_result.observed_views) > 1:
            for column, removed_view in enumerate(CANONICAL_VIEWS):
                ...
                omission_result = fuse_view_logits(logits_by_view, remaining)
